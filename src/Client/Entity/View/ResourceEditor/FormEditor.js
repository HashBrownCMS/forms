...
class FormEditor extends HashBrown.Entity.View.ResourceEditor.ResourceEditorBase {
    ...
    prerender() {
        ...
        this.state.postUrl = `${location.protocol}//${location.host}${this.context.config.system.rootUrl || ''}/api/${this.context.project.id}/${this.context.environment}/forms/${this.model.id}/submit`;
    ...
    async onClickClearEntries() {
        UI.confirm('Clear "' + this.model.getName() + '"', 'Are you sure you want to clear all entries?', async () => {
            await HashBrown.Service.RequestService.request('post', 'forms/' + this.model.id + '/clear');
